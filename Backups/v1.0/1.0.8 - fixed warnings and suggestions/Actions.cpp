...
static void PerformOcr_Thread(HBITMAP hBitmap, SnippingMode mode)
{
    ...
    try
    {
        ...
        {
            PostMessage(g_hMainWnd, WM_APP_SHOW_NOTIFICATION, NOTIFY_OCR_NOTEXT, 0);
        }
        else
        {
            ...
            if (mode == SnippingMode::OcrText)
            {
                CopyTextToClipboard(ss.str());
                PostMessage(g_hMainWnd, WM_APP_SHOW_NOTIFICATION, NOTIFY_OCR_SUCCESS, 0);
            }
            else if (mode == SnippingMode::OcrTextSave)
            {
                AppendOcrTextToFile(ss.str());
                PostMessage(g_hMainWnd, WM_APP_SHOW_NOTIFICATION, NOTIFY_OCR_SAVE_SUCCESS, 0);
            }
        }
    }
...
{
    std::thread(PerformOcr_Thread, hBitmap, mode).detach();
}