...
LRESULT CALLBACK ActionToolbarWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        HWND hBtnCopyImg = CreateWindowW(L"BUTTON", L"Copy Img", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, padding, padding, btnWidth, btnHeight, hWnd, (HMENU)ID_TOOLBAR_COPY_IMG, g_hInstance, NULL);
        HWND hBtnSaveImg = CreateWindowW(L"BUTTON", L"Save Img", WS_CHILD | WS_VISIBLE | BS_OWNERDRAW, btnWidth + 2 * padding, padding, btnWidth, btnHeight, hWnd, (HMENU)ID_TOOLBAR_SAVE_IMG, g_hInstance, NULL);
